...
// //         setFavorites(res.data.filter(r=> (r.savedBy||[]).length > 0))
...
// //         {favorites.length===0 ? <p className='text-slate-300'>No saved recipes yet.</p> : favorites.map(r=> (
// //           <div key={r._id} className='bg-white/3 p-4 rounded-xl'>
// //             <h4 className='font-semibold'>{r.title}</h4>
// //             <p className='text-sm text-slate-300'>By {r.author?.name}</p>
// //           </div>
// //         ))}
// //       </div>
// //     </div>
...
// export default function Profile() {
//   const [favorites, setFavorites] = useState([])
//   const [user, setUser] = useState({ name: 'Chef Srushti', bio: 'Passionate food lover exploring flavors around the world.' })
//   const token = localStorage.getItem('ff_token')
...
//             <p className="text-slate-300 mt-1 text-sm">{user.bio}</p>
...
//         {favorites.length === 0 ? (
//           <p className="text-slate-400 text-center italic">You haven’t saved any recipes yet. Explore and add some favorites!</p>
//         ) : (
...
//                 />
//                 <h4 className="font-semibold text-lg text-white mb-1">{r.title}</h4>
...
//                 </div>
//                 <p className="mt-2 text-xs italic text-slate-500">By {r.author?.name || 'Chef'}</p>
//               </div>
//             ))}
//           </div>
//         )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1c2c] via-[#23263b] to-[#1b1e31] text-white py-10 px-6">
      ...
      <div className="max-w-5xl mx-auto bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6 mb-12 shadow-xl">
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <img
            src={user.avatar || "https://cdn-icons-png.flaticon.com/512/1995/1995574.png"}
            alt="User Avatar"
            className="w-24 h-24 rounded-full border-2 border-white/20 shadow-lg"
          />
          <div className="text-center sm:text-left">
